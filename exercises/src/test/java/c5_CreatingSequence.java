...
public class c5_CreatingSequence {
    ...
    public void callable_counter() {
        ...
        StepVerifier.create(callableCounterMono.repeat(2))  //repeat two times after complete
                    .expectNext(1, 2, 3)
                    .verifyComplete();
    }
    ...
    public void future_counter() {
        ...
        CompletableFuture<Integer> completableFuture = CompletableFuture.supplyAsync(() -> {
            System.out.println("You are incrementing a counter via Future! " + this.getClass().getCanonicalName());
        ...
        StepVerifier.create(futureCounterMono.repeat(3))
                    .expectNext(1,1,1,1)
                    .verifyComplete();
    }
    ...
    public void trouble_maker() {
        Mono<String> trouble = Mono.delay(Duration.ofSeconds(4L)).error(new IllegalStateException("my trouble test")); //todo: change this line

        StepVerifier.create(trouble.timeout(Duration.ofSeconds(3L)))
    ...
    public void repeat() {
        ...
        Flux<Integer> repeated = Mono.fromCallable(() -> {
            ...
            System.out.print("next increment iteration: ");
            return val;
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> generateFlux = Flux.generate(() -> 0, // initial state
                (state, sink) -> {
                    sink.next(state); // emit value
                    if (state == 5) sink.complete(); // stop after 5

...
public class c8_Sinks extends SinksBase {
    ...
    public void single_subscriber() {
        //todo: feel free to change code as you need
        Sinks.Many<Integer> sink = Sinks.many().unicast().onBackpressureBuffer();//.replay().all();//.multicast().onBackpressureBuffer();
        Flux<Integer> measurements = sink.asFlux()
                //.cache() //НЕ нужен т.к. есть буфер
                .log()
                .doOnSubscribe(x -> System.out.println("Subscribtion activated: " + x))
                .doOnEach(x -> System.out.println("each: "+x))
            ...
            submitOperation(() -> {
                ...
                //sink.success(measures_readings);
            });

        //measurements = result.flatMapMany(Flux::fromIterable).cache();
        //don't change code below
    ...
    public void open_24_7() {
        ...
        Sinks.Many<Integer> sink = Sinks.many().multicast().onBackpressureBuffer(1000,false);
